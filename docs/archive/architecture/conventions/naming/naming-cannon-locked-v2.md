...
## 13. Event Document Naming

Some documents represent discrete events in system evolution.

These files must be saved as new files and must not be overwritten.
...
- `unix_ms` is canonical
- timestamp must be exact 13-digit unix milliseconds
- no rounding
- no truncation
- no padding
- human-readable time must be an exact expansion of the same unix_ms value

This rule applies only to event-level documents such as:
